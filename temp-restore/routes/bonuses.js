...
router.get('/', async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM bonuses ORDER BY created_at DESC LIMIT 1');
...
router.put('/', authenticateToken, requireRole('admin'), async (req, res) => {
    try {
        ...
        const existingBonuses = await pool.query('SELECT id FROM bonuses ORDER BY created_at DESC LIMIT 1');
        let result;
        if (existingBonuses.rows.length > 0) {
            // Обновляем существующие данные
            result = await pool.query('UPDATE bonuses SET title = $1, media = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $3 RETURNING *', [title, JSON.stringify(media || []), existingBonuses.rows[0].id]);
        }
        else {
            // Создаем новые данные
            result = await pool.query('INSERT INTO bonuses (title, media, created_by) VALUES ($1, $2, $3) RETURNING *', [title, JSON.stringify(media || []), userId]);
        }
        return res.json({
            success: true,
            data: result.rows[0]
