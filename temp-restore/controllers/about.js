...
export class AboutController {
    ...
    static async addMedia(req, res) {
        try {
            ...
            const { rows: result } = await pool.query('INSERT INTO about_media (about_id, filename, original_name, type, title, description, order_index, file_path) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *', [
                aboutId,
                mediaData.filename,
    ...
    static async updateMedia(req, res) {
        try {
            ...
            const values = Object.keys(updates)
                .filter(key => updates[key] !== undefined)
                .map(key => updates[key]);
            await pool.query(`UPDATE about_media SET ${updateFields} WHERE id = $${values.length + 1}`, [...values, id]);
            // Отправляем WebSocket уведомление
            if (wsManager) {
    ...
    static async reorderMedia(req, res) {
        try {
            ...
            const existingMedia = await pool.query('SELECT id FROM about_media WHERE id = ANY($1)', [mediaIds]);
            ...
            for (let i = 0; i < mediaIds.length; i++) {
                await pool.query('UPDATE about_media SET order_index = $1 WHERE id = $2', [i + 1, mediaIds[i]]);
            }
            // Отправляем WebSocket уведомление
            if (wsManager) {
