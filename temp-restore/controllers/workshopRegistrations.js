...
export const createWorkshopRegistration = async (req, res) => {
    try {
        console.log('=== НАЧАЛО createWorkshopRegistration ===');
        console.log('Request body:', JSON.stringify(req.body, null, 2));
        console.log('Request headers:', req.headers);
        const { workshopId, userId, style, options, totalPrice, notes } = req.body;
        ...
        const existingResult = await pool.query('SELECT id FROM workshop_registrations WHERE workshop_id = $1 AND user_id = $2', [workshopId, userId]);
        ...
        const workshopResult = await pool.query('SELECT participants FROM master_class_events WHERE id = $1', [workshopId]);
        if (workshopResult.rows.length > 0) {
            ...
            const isAlreadyParticipant = participants.some((p) => p.childId === userId);
        ...
        try {
            ...
            if (serviceResult.rows.length > 0) {
                ...
                const foundStyle = serviceStyles.find((s) => s.id === style);
                if (foundStyle) {
        ...
        try {
            ...
            const workshopResult = await pool.query('SELECT participants, statistics FROM master_class_events WHERE id = $1', [workshopId]);
            if (workshopResult.rows.length > 0) {
                ...
                if (updateParticipantsResult.rows.length === 0) {
                    ...
                    [styleName].forEach((styleNameStr) => {
...
export const createGroupWorkshopRegistration = async (req, res) => {
    try {
        console.log('=== НАЧАЛО createGroupWorkshopRegistration ===');
        console.log('Request body:', JSON.stringify(req.body, null, 2));
        console.log('Request headers:', req.headers);
        const { workshopId, parentId, children } = req.body;
        ...
        try {
            ...
            const parentResult = await client.query('SELECT name, surname FROM users WHERE id = $1', [parentId]);
            ...
            for (const child of children) {
                ...
                const existingResult = await client.query('SELECT id FROM workshop_registrations WHERE workshop_id = $1 AND user_id = $2', [workshopId, child.childId]);
                if (existingResult.rows.length > 0) {
                ...
                const workshopParticipantsResult = await client.query('SELECT participants FROM master_class_events WHERE id = $1', [workshopId]);
                if (workshopParticipantsResult.rows[0]?.participants) {
                    const participants = workshopParticipantsResult.rows[0].participants;
                    const isAlreadyParticipant = participants.some((p) => p.childId === child.childId);
                    if (isAlreadyParticipant) {
            ...
            const invoiceServiceResult = await client.query(`
                ...
                WHERE mce.id = $1
            `, [workshopId]);
            let invoiceServiceStyles = [];
            let invoiceServiceOptions = [];
            ...
            const selectedStyles = allStyles.map(styleId => {
                const style = invoiceServiceStyles.find((s) => s.id === styleId);
                return {
                    id: styleId,
            ...
            const selectedOptions = allOptions.map(optionId => {
                const option = invoiceServiceOptions.find((o) => o.id === optionId);
                return {
                    id: optionId,
            ...
            const participants = children.map((child, index) => {
                ...
                if (child.style) {
                    ...
                    selectedStyles = styleIds.map(styleId => {
                        const foundStyle = invoiceServiceStyles.find((s) => s.id === styleId);
                        return {
                            id: styleId,
                ...
                if (child.options && child.options.length > 0) {
                    selectedOptions = child.options.map(optionId => {
                        const foundOption = invoiceServiceOptions.find((o) => o.id === optionId);
                        return {
                            id: optionId,
            ...
            const currentStatsResult = await client.query('SELECT statistics FROM master_class_events WHERE id = $1', [workshopId]);
            ...
            // Получаем информацию о сервисе для извлечения названий стилей и опций
            const serviceResult = await client.query('SELECT styles, options FROM services WHERE id = (SELECT service_id FROM master_class_events WHERE id = $1)', [workshopId]);
...
export const getWorkshopStats = async (workshopId) => {
    try {
        ...
        const totalRevenue = registrations.reduce((sum, r) => sum + (parseFloat(String(r.total_price)) || 0), 0);
        // Получаем информацию о мастер-классе
...
export const checkRegistration = async (req, res) => {
    try {
        ...
        if (workshopResult.rows.length > 0) {
            const participants = workshopResult.rows[0].participants || [];
            isInParticipants = participants.some((p) => p.childId === userId);
        }
...
export const removeParticipant = async (req, res) => {
    ...
    try {
        ...
        const masterClassResult = await client.query('SELECT participants, statistics FROM master_class_events WHERE id = $1', [workshopId]);
        ...
        const registrationResult = await client.query('SELECT id FROM workshop_registrations WHERE workshop_id = $1 AND user_id = $2', [workshopId, participant.childId]);
        if (registrationResult.rows.length > 0) {
            await client.query('DELETE FROM workshop_registrations WHERE workshop_id = $1 AND user_id = $2', [workshopId, participant.childId]);
