 ...
 */
export const createIframePaymentData = async (req, res) => {
    try {
        ...
        if (result.success) {
            ...
            await pool.query('UPDATE invoices SET robokassa_invoice_id = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [result.iframeData?.invId, invoiceId]);
            res.json({
                success: true,
                data: {
...
export const createPaymentLink = async (req, res) => {
    try {
        ...
        if (result.success) {
            // Сохраняем ID счета Robokassa в БД
            await pool.query('UPDATE invoices SET robokassa_invoice_id = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [result.invoiceId, invoiceId]);
            res.json({
                success: true,
                data: {
 ...
 */
export const handleResultNotification = async (req, res) => {
    try {
        ...
        const invoiceResult = await pool.query('SELECT * FROM invoices WHERE robokassa_invoice_id = $1 OR id = $1', [InvId]);
        if (invoiceResult.rows.length === 0) {
        ...
        // Это требование ФЗ-54 для операций с предоплатой
        try {
            const secondReceiptResult = await robokassaService.createSecondReceipt({
                merchantId: process.env.ROBOKASSA_MERCHANT_LOGIN || 'waxhands.ru',
                id: `${invoice.id}_second`,
                originId: InvId.toString(),
                total: parseFloat(OutSum),
                items: [{
                        Name: `Мастер-класс (итоговый чек)`,
                        Quantity: 1,
                        Cost: parseFloat(OutSum),
                        Tax: "none",
                        PaymentMethod: "full_payment", // Для второго чека только full_payment
                        PaymentObject: "service"
                    }],
                clientEmail: invoice.user_email,
                clientPhone: invoice.user_phone
            });
            if (secondReceiptResult.success) {
                console.log('✅ Второй чек успешно создан');
            }
            else {
 ...
 */
export const handleJWSNotification = async (req, res) => {
    try {
        ...
        // Находим счет по invId
        const invoiceResult = await pool.query('SELECT * FROM invoices WHERE robokassa_invoice_id = $1 OR id = $1', [data.invId]);
        if (invoiceResult.rows.length === 0) {
...
export const createRefund = async (req, res) => {
    try {
        ...
        if (refundResult.success) {
            // Обновляем статус счета
            await pool.query('UPDATE invoices SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', ['cancelled', invoiceId]);
            res.json({
                success: true,
                data: {
...
//# sourceMappingURL=robokassaController.js.map