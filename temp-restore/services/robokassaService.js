...
export class RobokassaService {
    ...
    constructor() {
        this.config = {
            merchantLogin: process.env.ROBOKASSA_MERCHANT_LOGIN || 'test',
     ...
     */
    createJWTSignature(data, secretKey) {
    ...
    createReceipt(data) {
        ...
        // Согласно документации Robokassa, для автоматической фискализации
        // необходимо передавать Receipt в формате JSON
        const receipt = {
    ...
    async createInvoice(data) {
        try {
            ...
            // Создаем уникальный ID счета
            const invId = parseInt(data.invoiceId.replace(/-/g, '').substring(0, 10), 16);
            // При автоматической фискализации передаем Receipt в форме
            // Robokassa автоматически создает чеки и отправляет в ФНС
            console.log('🧾 Автоматическая фискализация - Receipt передается в форме');
            // Создаем фискальный чек для автоматической фискализации
            const receipt = this.createReceipt(data);
            // Правильная подпись согласно документации
            // Для тестового режима: MerchantLogin:OutSum:InvId:Пароль#1
            // Для продакшн с фискализацией: MerchantLogin:OutSum:InvId:Receipt:Пароль#1
            let signatureString;
            if (this.config.testMode) {
                // Для тестового режима используем упрощенную подпись без Receipt
                signatureString = `${this.config.merchantLogin}:${data.amount}:${invId}:${this.config.password1}`;
                console.log('🧪 Тестовый режим - подпись БЕЗ Receipt:', signatureString);
            }
            else {
                // Для продакшн режима включаем Receipt в подпись
                signatureString = `${this.config.merchantLogin}:${data.amount}:${invId}:${receipt}:${this.config.password1}`;
                console.log('🏪 Продакшн режим - подпись С Receipt:', signatureString);
            }
            const signature = this.createSignature(signatureString, '', true); // Используем MD5
            console.log('🔍 Полученная подпись:', signature);
            // Создаем HTML форму для POST запроса
            const formData = {
                MerchantLogin: this.config.merchantLogin,
            ...
            // Добавляем Receipt только для продакшн режима
            if (!this.config.testMode) {
                formData.Receipt = encodeURIComponent(receipt);
            }
    ...
    verifyResultSignature(notification) {
        try {
            ...
            // Сортируем Shp параметры по алфавиту (согласно документации)
            const sortedShpParams = Object.keys(shpParams)
                .filter(key => key.toLowerCase().startsWith('shp_'))
                .sort()
                .map(key => `${key}=${shpParams[key]}`); // Формат: Shp_param=value
     ...
     */
    async checkOperationStatus(invoiceId) {
        try {
            ...
            return new Promise((resolve) => {
                parseString(xmlText, (err, result) => {
                    ...
                    try {
                        ...
                        const operationStateResponse = parsedResult['OperationStateResponse'];
                        const resultInfo = operationStateResponse['Result']?.[0];
                        if (!resultInfo) {
                            resolve({
                                success: false,
                        ...
                        const stateInfo = operationStateResponse['State']?.[0];
                        const info = operationStateResponse['Info']?.[0];
                        if (!stateInfo || !info) {
                            resolve({
                                success: false,
                                error: 'Неверный формат XML ответа'
