...
## Задача: Очистка старых архивов деплоя и .gitignore
- **Статус**: ✅ Завершена
- **Дата**: 2026-02-11
- **Описание**: Удалить из репозитория устаревшие архивы деплоя (backend-*.zip, frontend-update-*, backend-update-* в корне и в backend/), добавить паттерны в .gitignore, проверить сборку.
- **Шаги выполнения**:
  - [x] Добавить в .gitignore паттерны для архивов деплоя
  - [x] Удалить архивы в корне и в backend/
  - [x] Проверить сборку frontend и backend
  - [x] Обновить changelog.md и Tasktracker.md
- **Результат**: Архивы удалены, .gitignore обновлён, сборки проходят успешно.

## Задача: Управление доступом к регистрации/входу с лендинга
- **Статус**: ✅ Завершена
- **Дата**: 2026-01-19
- **Описание**: Создать возможность для админа включать/отключать кнопки регистрации и входа на лендинге через админ-панель. Добавить переключатель на вкладке "Обзор" в админ-панели.
- **Шаги выполнения**:
  - [x] Создать миграцию для таблицы `landing_settings`
  - [x] Создать database модуль `landingSettings.ts`
  - [x] Создать контроллер `landingSettings.ts`
  - [x] Создать роуты `landingSettings.ts` с публичным и защищенным endpoints
  - [x] Добавить роуты в `index.ts`
  - [x] Добавить методы API в `api.ts`
  - [x] Создать хук `use-landing-settings.ts` для публичного и админского использования
  - [x] Добавить переключатель в админ-панели на вкладке "Обзор"
  - [x] Обновить `landing-header.tsx` для проверки настройки
  - [x] Обновить `HeroSection.tsx` для проверки настройки
  - [x] Обновить `ServicesSection.tsx` для проверки настройки
  - [x] Обновить `CTASection.tsx` для проверки настройки
  - [x] Обновить `DeliveryPaymentSection.tsx` для проверки настройки
  - [ ] Применить миграцию на сервере
- **Зависимости**:
  - Backend: `backend/migrations/add-landing-settings.sql`, `backend/src/database/landingSettings.ts`, `backend/src/controllers/landingSettings.ts`, `backend/src/routes/landingSettings.ts`
  - Frontend: `src/hooks/use-landing-settings.ts`, `src/lib/api.ts`, `src/pages/admin/Dashboard.tsx`, `src/components/ui/landing-header.tsx`, `src/components/landing/*.tsx`
- **Результат**:
  - ✅ Создана таблица `landing_settings` в БД
  - ✅ Добавлен API для получения и обновления настроек (публичный и защищенный)
  - ✅ Добавлен переключатель в админ-панели на вкладке "Обзор"
  - ✅ Все компоненты лендинга проверяют настройку перед показом кнопок
  - ✅ Админ может включать/отключать доступ к регистрации через админ-панель
- **Примечание**: 
  - По умолчанию `registration_enabled = FALSE` (регистрация отключена)
  - Публичный endpoint `/landing-settings/public` доступен без авторизации для лендинга
  - Защищенный endpoint `/landing-settings` требует авторизации и роли админа
- **Миграция**: 
  - Файл: `backend/migrations/add-landing-settings.sql`
  - Команда: `psql -U postgres -d waxhands -f backend/migrations/add-landing-settings.sql`

## Задача: Создание автоматизированных скриптов деплоя v2
- **Статус**: ✅ Завершена
- **Дата**: 2026-01-19
- **Описание**: Изучить текущий процесс обновления frontend и backend на сервере через SSH, проанализировать использование папки dist и локального копирования через SCP, создать два улучшенных PowerShell скрипта для автоматизации деплоя с полной документацией.
- **Шаги выполнения**:
  - [x] Подключиться к серверу через SSH (root@147.45.161.83)
  - [x] Изучить структуру папок frontend и backend на сервере
  - [x] Проверить использование папки dist vs корневые файлы
  - [x] Проанализировать Nginx конфигурацию
  - [x] Изучить PM2 процессы
  - [x] Проанализировать существующие скрипты update-frontend.ps1 и update-backend.ps1
  - [x] Создать улучшенный скрипт `deploy-frontend-v2.ps1`
  - [x] Создать улучшенный скрипт `deploy-backend-v2.ps1`
  - [x] Добавить валидацию и проверку файлов
  - [x] Добавить автоматическое создание резервных копий для backend
  - [x] Добавить механизм отката при ошибках
  - [x] Добавить цветной вывод для лучшей читаемости
  - [x] Добавить подробную диагностику и логирование
  - [x] Создать полное руководство `DEPLOY.md`
  - [x] Обновить документацию (`changelog.md`, `tasktracker.md`)
- **Зависимости**:
  - Скрипты: `deploy-frontend-v2.ps1`, `deploy-backend-v2.ps1`
  - Документация: `DEPLOY.md`
  - Сервер: SSH доступ к root@147.45.161.83
  - Инструменты: PowerShell 5.1+, tar, scp, ssh
- **Результат**:
  - ✅ Проанализирована структура сервера:
    - Frontend: Файлы хранятся напрямую в `/var/www/waxhands-app/frontend/` (без dist)
    - Backend: Файлы хранятся в `/var/www/waxhands-app/backend/dist/`
    - Nginx root: `/var/www/waxhands-app/frontend`
    - PM2 процесс: `waxhands-backend` на порту 3001
  - ✅ Создан `deploy-frontend-v2.ps1`:
    - 8 шагов с подробным выводом
    - Валидация JS файлов и размеров
    - Полная очистка frontend папки на сервере
    - Автоматическая очистка старых архивов (оставляет 3)
    - Использование tar.gz для совместимости с Linux
  - ✅ Создан `deploy-backend-v2.ps1`:
    - 9 шагов с подробным выводом
    - Проверка ключевых файлов (index.js, websocket-server.js)
    - Автоматическое создание бэкапа с timestamp
    - Автоматический откат при ошибках
    - Проверка работоспособности API
    - Очистка старых бэкапов (оставляет 5)
  - ✅ Создано `DEPLOY.md`:
    - Описание структуры сервера
    - Предварительные требования
    - Пошаговые инструкции для 3 сценариев
    - Раздел диагностики и отката
    - FAQ с частыми проблемами
    - Лучшие практики
- **Примечание**: 
  - Скрипты v2 заменяют старые update-*.ps1 и используют tar.gz вместо zip
  - Frontend не имеет автоматических бэкапов (используйте Git для отката)
  - Backend создает автоматические бэкапы перед каждым обновлением
  - Все команды адаптированы для Windows PowerShell

## Задача: Добавление возможности отключения оплаты для конкретной школы
- **Статус**: ✅ Завершена
- **Дата**: 2025-11-24
- **Описание**: Добавить возможность отключать оплату для конкретной школы в админ панели. При отключении оплаты для школы, вместо кнопки "Оплатить" должно отображаться сообщение с кликабельными номерами WhatsApp.
- **Шаги выполнения**:
  - [x] Создать миграцию для добавления поля `payment_disabled` в таблицу `schools`
  - [x] Добавить метод `toggleSchoolPayment` в контроллер школ
  - [x] Добавить маршрут `PATCH /schools/:id/payment` в роутер школ
  - [x] Добавить метод `schools.togglePayment` в API клиент
  - [x] Обновить тип `School` для включения поля `paymentDisabled`
  - [x] Добавить кнопку "Выключить оплату" / "Подключить оплату" в админ панели для каждой школы
  - [x] Обновить компонент оплаты для проверки статуса школы
  - [x] Добавить сообщение с кликабельными номерами WhatsApp при отключенной оплате
  - [x] Проверить линтер и убедиться в отсутствии ошибок
  - [x] Обновить документацию (`changelog.md`, `tasktracker.md`)
- **Зависимости**:
  - Backend: `backend/migrations/add-school-payment-disabled.sql`, `backend/src/controllers/schools.ts`, `backend/src/routes/schools.ts`
  - Frontend: `src/components/admin/MasterClassesTab.tsx`, `src/components/ui/yandex-payment-button.tsx`, `src/types/index.ts`, `src/lib/api.ts`
- **Результат**:
...
- **Примечание**: При отключении оплаты для школы, родители видят сообщение "Оплата на данный мастер-класс закрыта. Для оплаты напишите в WhatsApp +79145470606 или +79145450606" вместо кнопки оплаты.
- **Деплой**: 
  - Миграция применена: `psql -U postgres -d waxhands -f /tmp/add-school-payment-disabled.sql`
  - Backend обновлен: `backend-update-20251124-200340.tar.gz`
  - Frontend обновлен: `frontend-update-20251124-200518.tar.gz`
  - Скрипты обновлены для использования tar.gz вместо zip (решение проблемы с обратными слешами в Windows)
