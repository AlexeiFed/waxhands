...
- **Мобильная админка:** дорожная карта (docs/дорожная-карта.md), шаг 1 аудит (docs/mobile-admin-step-1-audit.md), шаг 2 дизайн навигации/карточек (docs/mobile-admin-step-2-design.md), шаг 3 план компонентов + прогресс (docs/mobile-admin-step-3-components.md)
...
### Структура API
### Мобильная админка — Активные компоненты

- **ResponsiveLayoutProvider** (`src/contexts/ResponsiveLayoutContext.tsx`) — глобальный провайдер брейкпоинтов, используется в мобильных компонентах навигации и списков.
- **MobileAppBar / MobileAdminDrawer / FloatingActionButton** (`src/components/admin/navigation/`) — мобильная навигация и быстрые действия.
- **Dashboard (мобильная шапка)** (`src/pages/admin/Dashboard.tsx`) — MobileAppBar, Drawer и FloatingActionButton подключены при `isSmallScreen`, табы синхронизированы с гамбургером, десктопный хедер сохранён.
- **Dashboard (быстрые ссылки и действия)** (`src/pages/admin/Dashboard.tsx`) — мобильная вкладка Overview содержит carousel быстрых ссылок и вертикальный блок быстрых действий.
- **Dashboard (статистика)** (`src/pages/admin/Dashboard.tsx`) — collapsible-секция ключевых метрик (mobile: top-3 по умолчанию, desktop: раскрыто) с адаптивной сеткой 1/2/3/4/5 колонок.
- **ResponsiveList** (`src/components/admin/lists/ResponsiveList.tsx`) — универсальный список для карточного представления данных с поддержкой skeleton/empty состояний.
- **AdminCardBase** (`src/components/admin/cards/AdminCardBase.tsx`) — базовая карточка с selectable состояниями и тематическими тонами.
- **UserCard** (`src/components/admin/cards/UserCard.tsx`) — карточка пользователя (контакты, роль, дети) на базе AdminCardBase.
- **UsersTab (мобильный режим)** — `src/pages/admin/Dashboard.tsx`: вкладка «Пользователи» на мобильных устройствах использует `ResponsiveList` + `UserCard`, десктопная таблица сохранена.
- **SchoolCard** (`src/components/admin/cards/SchoolCard.tsx`) — карточка школы с контактами, классами и действиями; мобильный таб «Школы» в `Dashboard` переключается на `ResponsiveList` + `SchoolCard`.
- **MasterClassesTab (мобильный режим)** — `src/components/admin/MasterClassesTab.tsx`: мобильный список мастер-классов отображается через `ResponsiveList` + `MasterClassCard`, десктопная группировка по школам сохранена.
- **MasterClassesTab (управление оплатой)** — глобальный переключатель Robokassa закреплён в шапке вкладки рядом с «Создать мастер-класс», на мобильных элементы складываются в несколько строк благодаря `flex-wrap`, состояние синхронизируется через WebSocket событие `payment_settings_changed`.
- **InvoicesTab (мобильный режим)** — `src/components/admin/InvoicesTab.tsx`: список счетов на мобильных отображается `ResponsiveList` + `InvoiceCard`, десктопная таблица сохранена.
- **WorkshopRequestsTab (мобильный режим)** — `src/components/admin/WorkshopRequestsTab.tsx`: заявки выводятся карточками `RequestCard` с поддержкой изменения статуса и удаления.
- **AdminChat (мобильный режим)** — `src/components/ui/admin-chat.tsx`: список диалогов `ResponsiveList` + `ChatDialogCard`, контент чата стекается под списком.
- **InvoiceCard** (`src/components/admin/cards/InvoiceCard.tsx`) — карточка счёта (статус, сумма, платёжные детали); `InvoicesTab` на мобильных использует `ResponsiveList` + `InvoiceCard`.
- **RequestCard** (`src/components/admin/cards/RequestCard.tsx`) — карточка заявки на мастер-класс (родитель, школа, заметки); `WorkshopRequestsTab` отображает мобильный список через `ResponsiveList`.
- **ChatDialogCard** (`src/components/admin/cards/ChatDialogCard.tsx`) — карточка диалога чата; `src/components/ui/admin-chat.tsx` на мобильных использует `ResponsiveList` + `ChatDialogCard`.
- **SelectionManagerProvider** (`src/contexts/SelectionManagerContext.tsx`) + `BulkActionBar` — единое API для множественного выбора и массовых действий в мобильных списках.
- **FilterChips / FilterDrawer** (`src/components/admin/filters/`) — компактные чипы активных фильтров и мобильный bottom sheet для управления фильтрами.
...
- **WebSocket синхронизация** — событие `invoice_update` содержит `masterClassId`, рассылается родителям и админам; `src/hooks/use-invoices-websocket.ts` умеет слушать все события в админке и инвалидирует списки счетов.
- **Realtime доставка** — `backend/src/websocket-server.ts` логирует каждое уведомление и дополнительно шлёт его через каналы `admin:all`/`system:all`, чтобы администраторы получали обновление даже при общих WebSocket-подключениях.
- **Синхронизация участников** — `syncPaymentStatusWithParticipants` из `invoices.ts` вызывается из Robokassa контроллера, помечая `participants[].isPaid` и пересчитывая статистику сразу после оплаты.
- **Глобальные настройки оплаты** — новое событие `payment_settings_changed` уведомляет родителей и админов без костылей через `master_class_update`; фронтенд ловит его через `use-payment-settings`.
- **Robokassa на iOS** — `src/components/payment/RobokassaPayment.tsx` заранее открывает окно и переиспользует его при отправке формы, обходя блокировки Safari; fallback в том же окне остаётся для PWA-режима.
- **Админ-панель** — `InvoicesTab` показывает тосты при обновлении статуса, таблица `RefundsTab` дополнена колонкой e-mail для ручной обработки возвратов.
- **Ручные возвраты** — `RefundReasonModal` запрашивает e-mail, `backend/src/controllers/robokassaController.ts` сохраняет заявку (статус `pending`, `refund_email`) и рассылает уведомления; миграция `backend/src/database/add-refund-email-field.ts` добавляет поле `refund_email` в таблицу `invoices`. Таблица `/api/admin/refunds` возвращает только статусы `pending/completed/cancelled`, UI выводит суммы и причину в читаемом виде.
- **Документация** — см. `docs/changelog.md`, `docs/tasktracker.md` для детализации шагов и зависимостей.
...
- **Карточка заявки**: блок подачи заявки на проведение отображается только на вкладке «Мастер-классы» и когда нет доступных мастер-классов
...
- **Контакты родителей**: имя и телефон родителя отображаются в карточке и таблице (tel-ссылки)
- **Автообновление**: после наличных и онлайн оплат данные подтягиваются через WebSocket, кнопка «Обновить» принудительно перезапрашивает участников
