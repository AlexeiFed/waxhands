...
router.get('/', async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM contacts ORDER BY created_at DESC LIMIT 1');
...
router.put('/', authenticateToken, requireRole('admin'), async (req, res) => {
    try {
        const { company_name, legal_status, inn, phone, email, address, website } = req.body;
        ...
        const existingContacts = await pool.query('SELECT id FROM contacts ORDER BY created_at DESC LIMIT 1');
        let result;
        if (existingContacts.rows.length > 0) {
            // Обновляем существующие данные
            result = await pool.query('UPDATE contacts SET company_name = $1, legal_status = $2, inn = $3, phone = $4, email = $5, address = $6, website = $7, updated_at = CURRENT_TIMESTAMP WHERE id = $8 RETURNING *', [company_name, legal_status, inn, phone, email, address || '', website || '', existingContacts.rows[0].id]);
        }
        else {
            // Создаем новые данные
            result = await pool.query('INSERT INTO contacts (company_name, legal_status, inn, phone, email, address, website, created_by) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *', [company_name, legal_status, inn, phone, email, address || '', website || '', userId]);
        }
        return res.json({
            success: true,
            data: result.rows[0]
