...
export class RobokassaService {
    ...
    async createInvoice(data: CreateRobokassaInvoiceData): Promise<RobokassaCreateInvoiceResponse> {
        try {
            ...
            // Формируем shp-параметры (ПРАВИЛЬНО - только значения)
            const shpValues = [
                data.invoiceId,
                encodeURIComponent(data.participantName || 'неизвестен')
            ].join(':');

            // ПРАВИЛЬНАЯ подпись согласно документации Robokassa
            // MerchantLogin:OutSum:InvId:Password1:shp_value1:shp_value2
            const signatureString = `${this.config.merchantLogin}:${outSum}:${invId}:${this.config.password1}:${shpValues}`;
            ...
            const formData: {
                ...
                Culture: 'ru',
                Encoding: 'utf-8',
                // Добавляем shp-параметры (в форме они с префиксом)
                shp_invoice_id: data.invoiceId,
                shp_participant: data.participantName || 'неизвестен'
            };
