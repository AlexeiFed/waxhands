...
export class PaymentChecker {
    ...
    private checkInterval: number = 30; // секунды
     ...
     */
    async checkAllPendingPayments(): Promise<PaymentCheckResult> {
        try {
            console.log('🔍 Проверяем все pending счета...');

            // Получаем все pending счета
            const result = await pool.query(
                'SELECT id, payment_label, amount FROM invoices WHERE status = $1 ORDER BY created_at DESC',
                ['pending']
            );
     ...
     */
    async checkInvoicePayment(invoiceId: string): Promise<PaymentCheckResult> {
        try {
            console.log(`🔍 Проверяем счет ${invoiceId}...`);

            // Получаем информацию о счете
            const result = await pool.query(
                'SELECT id, payment_label, amount, status FROM invoices WHERE id = $1',
                [invoiceId]
            );
