...
export const setWebSocketManager = (manager) => {
    webSocketManager = manager;
};
/**
 * Отправляет уведомление пользователю через WebSocket
 */
export const sendWebSocketNotification = async (notificationData) => {
    try {
 ...
 */
export const sendPaymentSuccessNotification = async (userId, invoiceId, amount, paymentMethod) => {
 ...
 */
export const sendPaymentFailedNotification = async (userId, invoiceId, error) => {
    const notificationData = {
        userId,
        type: 'payment_failed',
        title: 'Ошибка оплаты',
 ...
 */
export const sendPaymentReceivedNotification = async (adminUserId, invoiceId, amount, sender) => {
    const notificationData = {
        userId: adminUserId,
...
export const getUserNotifications = async (userId, limit = 50) => {
    try {
        const result = await pool.query(`SELECT * FROM user_notifications 
             WHERE user_id = $1 
...
export const markNotificationAsRead = async (notificationId, userId) => {
    try {
        await pool.query('UPDATE user_notifications SET read = true WHERE id = $1 AND user_id = $2', [notificationId, userId]);
        return true;
    }
    catch (error) {
 ...
 */
export const sendInvoiceStatusUpdate = async (invoiceId, newStatus, userId) => {
    try {
        const manager = getWebSocketManager();
        if (manager) {
            const message = {
