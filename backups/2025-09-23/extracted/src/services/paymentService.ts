 ...
 */
export const updateInvoicePaymentStatus = async (paymentData: PaymentData): Promise<PaymentResult> => {
    ...
    try {
        ...
        const updateResult = await client.query(
            `UPDATE invoices 
             ...
             WHERE id = $5 
             RETURNING *`,
            ['paid', paymentData.paymentId, paymentData.paymentMethod, paymentData.paymentDate, paymentData.invoiceId]
        );
 ...
 */
export const findInvoiceByLabel = async (label: string): Promise<string | null> => {
    try {
        console.log(`🔍 Ищем счет по метке: "${label}"`);

        const result = await pool.query(
            'SELECT id FROM invoices WHERE payment_label = $1 AND status = $2',
            [label, 'pending']
        );
        ...
        if (result.rows.length > 0) {
            ...
            const allResult = await pool.query(
                'SELECT id, payment_label, status FROM invoices WHERE payment_label IS NOT NULL ORDER BY created_at DESC LIMIT 3',
                []
            );
 ...
 */
export const findInvoiceBySender = async (sender: string): Promise<string | null> => {
    try {
        const result = await pool.query(
            'SELECT id FROM invoices WHERE sender_phone = $1 AND status = $2',
            [sender, 'pending']
        );
