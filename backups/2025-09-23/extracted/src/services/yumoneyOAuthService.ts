...
interface YuMoneyTokenResponse {
    access_token: string;
    token_type: string;
    expires_in: number;
...
export class YuMoneyOAuthService {
    ...
    private async getAccessToken(): Promise<string> {
        ...
        try {
            ...
            const tokenData: YuMoneyTokenResponse = response.data;
     ...
     */
    async getPaymentInfo(operationId: string): Promise<YuMoneyPaymentInfo | null> {
        try {
     ...
     */
    async createPaymentForm(invoiceId: string, amount: number, description: string): Promise<string> {
        try {
            ...
            const result = await pool.query(
                'SELECT payment_label FROM invoices WHERE id = $1',
                [invoiceId]
            );
     ...
     */
    async checkPaymentByLabel(label: string): Promise<YuMoneyPaymentInfo | null> {
        try {
            const accessToken = await this.getAccessToken();
     ...
     */
    private mapStatus(yumoneyStatus: string): 'success' | 'pending' | 'failed' {
