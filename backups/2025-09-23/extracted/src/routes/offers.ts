...
import { Offer, CreateOfferRequest, UpdateOfferRequest, OfferFilters } from '../types/offers';
...
router.get('/', async (req, res) => {
    try {
        const { is_active, version, created_by } = req.query as OfferFilters;
...
router.post('/', authenticateToken, requireRole('admin'), async (req, res) => {
    try {
        const { title, content, version }: CreateOfferRequest = req.body;
        ...
        const existingOffer = await pool.query(
            'SELECT id FROM offers WHERE version = $1',
            [version]
        );
        ...
        const result = await pool.query(
            'INSERT INTO offers (title, content, version, created_by) VALUES ($1, $2, $3, $4) RETURNING *',
...
router.put('/:id', authenticateToken, requireRole('admin'), async (req, res) => {
    try {
        const { id } = req.params;
        const { title, content, version }: UpdateOfferRequest = req.body;
        ...
        if (version && version !== existingOffer.rows[0].version) {
            const versionCheck = await pool.query(
                'SELECT id FROM offers WHERE version = $1 AND id != $2',
                [version, id]
            );
        ...
        const result = await pool.query(
            `UPDATE offers SET ${updateFields.join(', ')} WHERE id = $${paramCount} RETURNING *`,
            [...params, id]
        );
...
router.patch('/:id/activate', authenticateToken, requireRole('admin'), async (req, res) => {
    try {
        ...
        await pool.query(
            'UPDATE offers SET is_active = false, updated_at = CURRENT_TIMESTAMP'
        );

        // Активируем выбранную оферту
        const result = await pool.query(
            'UPDATE offers SET is_active = true, updated_at = CURRENT_TIMESTAMP WHERE id = $1 RETURNING *',
            [id]
        );
