...
const addParticipantToMasterClass = async (client, invoice) => {
    ...
    const parentResult = await client.query('SELECT name, surname FROM users WHERE id = $1', [invoice.participant_id]);
    ...
    const masterClassCheckResult = await client.query('SELECT id FROM master_class_events WHERE id = $1', [invoice.master_class_id]);
    ...
    const verifyResult = await client.query('SELECT participants FROM master_class_events WHERE id = $1', [invoice.master_class_id]);
    ...
    const currentStatsResult = await client.query('SELECT statistics FROM master_class_events WHERE id = $1', [invoice.master_class_id]);
...
export const createInvoice = async (req, res) => {
    try {
        ...
        try {
            ...
            const verifyInvoiceResult = await client.query('SELECT * FROM invoices WHERE id = $1', [newInvoice.id]);
            ...
            const afterCommitResult = await client.query('SELECT * FROM invoices WHERE id = $1', [newInvoice.id]);
    ...
    try {
        ...
        const result = await pool.query(`UPDATE invoices 
             SET selected_styles = $1, 
                 ...
                 updated_at = CURRENT_TIMESTAMP 
             WHERE id = $4 
             RETURNING *`, [JSON.stringify(selected_styles), JSON.stringify(selected_options), amount, id]);
...
export const updateInvoiceStatus = async (req, res) => {
    try {
        ...
        const result = await pool.query('UPDATE invoices SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 RETURNING *', [status, id]);
        if (result.rows.length === 0) {
            console.error(`❌ Счет с ID ${id} не найден`);
            res.status(404).json({
                success: false,
...
const syncPaymentStatusWithParticipants = async (masterClassId, participantId, isPaid) => {
    try {
        ...
        if (participantsToUpdate.length === 0) {
            ...
            console.log(`❌ Доступные parentId:`, participants.map((p) => p.parentId));
            console.log(`❌ Доступные childId:`, participants.map((p) => p.childId));
            console.log(`❌ Доступные notes:`, participants.map((p) => p.notes));
            // Попробуем найти участников по ID счета в notes
            const participantsByInvoice = participants.filter((p) => p.notes && p.notes.includes(`Счет: ${participantId}`));
            if (participantsByInvoice.length > 0) {
                console.log(`🔍 Найдено участников по ID счета в notes: ${participantsByInvoice.length}`);
                console.log(`🔍 Участники по ID счета:`, participantsByInvoice.map((p) => ({ id: p.id, childName: p.childName || 'Без имени' })));
        ...
        participants = participants.map((p) => {
            const isTargetParticipant = participantsToUpdate.some((targetP) => targetP.id === p.id);
        ...
        const updateResult = await pool.query(`UPDATE master_class_events 
             SET participants = $1::jsonb, 
                 ...
                 updated_at = CURRENT_TIMESTAMP
             WHERE id = $4
             RETURNING *`, [JSON.stringify(participants), totalPaidAmount, totalUnpaidAmount, masterClassId]);
...
export const syncAllInvoicesWithParticipants = async (req, res) => {
    try {
        ...
        for (const invoice of invoices) {
            try {
                ...
                await syncPaymentStatusWithParticipants(invoice.master_class_id, invoice.participant_id, invoice.status === 'paid');
                syncedCount++;
